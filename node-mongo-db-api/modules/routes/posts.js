exports.posts = (app, client, database) => {
    ...
    app.get('/posts/get', async (req, res) => {
        console.log("DEBUG: Chiamato endpoint /posts/get")
    ...
    app.get('/posts/get/:id', async (req, res) => {
        console.log("DEBUG: Chiamato endpoint /posts/get/:id")
        const authenticate = await auth.authentication(client, database, req);
    ...
    app.post('/posts/add', async (req, res) => {
        console.log("DEBUG: Chiamato endpoint /posts/add")
        ...
        if (authenticate === 200) {
            ...
            try {
                console.log(`DEBUG: req.body: ${req.body}`, req.body)

                const randomId = Math.floor((1 + Math.random()) * 0x10000).toString(16).substring(1);

                const collection = database.collection('data');

                const result = await collection.updateOne({ email: req.headers['email'] }, {
                    $push:
